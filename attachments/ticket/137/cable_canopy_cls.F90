...
!          of last 3 changes.
!        : revision of latent_heat_flux subroutine to ensure consistency in use of
!          latent heat of evaporation/sublimation between _canopy and _soilsnow  
! ==============================================================================
...
CONTAINS
  ...
  CONTAINS
    ...
    SUBROUTINE Latent_heat_flux()
      ...
      !             i) evaporationfrom /dew onto surfaces with no snow and T>freezing
      !            ii) sublimation from/frost onto surfaces with snow cover
      !           iii) evaporation ofliquid water in soil if no snow but frozen soil
      !            iv) deposition of frost onto frozen surfaces with no/little snow
      !
      ! Importantly the value of %cls set here is now used to control 
      ! whether water fluxes are from the snow pack or soil columnn in _soilsnow 
      ...
      DO j=1,mp
         ...
         IF(ssnow%snowd(j) < 0.1 .AND. canopy%fess(j) .GT. 0. ) THEN
            ...
            !Ticket 137 - case iii) 
            !evaporation from frozen soils needs to respect assumption that the solid 
            !fraction of moisture cannot exceed 85% - see frozen_limit in cable_soilsnow
            ! if frozen_limit is altered the corresponding change needs to be made here.
         ...
         !Ticket 137 - case iv) deposition of frost onto frozen soil with no snow
         ...
         !Ticket 137 - case ii) evaporation of snow
